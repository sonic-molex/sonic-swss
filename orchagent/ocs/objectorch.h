...
typedef sai_status_t (*BulkCreateObjectFunc)(
...
typedef sai_status_t (*BulkRemoveObjectFunc)(
        uint32_t object_count,
...
public:
    ...
    bool bulkCreateObjects(const std::vector<std::string> &keys, size_t batchTotal = 0);
    bool bulkRemoveObjects(const std::vector<std::string> &keys);

    virtual std::vector<std::string> collectInvalidConnections();
    virtual void cleanupInvalidConnections(const std::vector<std::string> &invalidKeys);
    ...
    bool loadFlexCounterScript(_In_ const std::string &script_path,
    ...
    void addId(const std::string &key, std::map<std::string, std::string> &attrs);
...
protected:
    ...
    std::unique_ptr<Table> m_cfgDbTable;
    ...
    BulkCreateObjectFunc m_bulkCreateFunc = nullptr;

    BulkRemoveObjectFunc m_bulkRemoveFunc = nullptr;
    ...
    /**
     * Attributes that can be modified by  at anytime.
     */
    ...
    /**
     * Irrecoverable attributes like _LINECARD_ATTR_RESET.
     * These attributes are modified by notification channels instead of config table.
     */

    std::map<std::string, sai_attr_id_t> m_irrecoverableAttrs;
    ...
    std::map<std::string, sai_stat_id_t> m_statValues;
